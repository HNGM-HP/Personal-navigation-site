...
switch ($action) {
    case 'get_all_data':
        ...
        // Pre-calculate folder sort weights for bookmark sorting
        $folderWeights = [];
        foreach ($folders as $f) {
            $folderWeights[$f['id']] = (int)($f['sort_order'] ?? 0);
        }

        // Sort bookmarks by folder order, then bookmark sort_order, then add_date (DESC)
        usort($bookmarks, function($a, $b) use ($folderWeights) {
            $fA = $folderWeights[$a['folder_id'] ?? ''] ?? 999999;
            $fB = $folderWeights[$b['folder_id'] ?? ''] ?? 999999;
    ...
    case 'batch_delete_bookmarks':
        log_debug('Executing action: batch_delete_bookmarks');
        $ids = $input['ids'] ?? [];
        if (empty($ids)) {
            jsonResponse(['status' => 'error', 'message' => '没有提供要删除的 ID'], 400);
        }
        $bookmarksData = readJsonFile($bookmarkFile);
        $before = count($bookmarksData);
        $bookmarksData = array_values(array_filter($bookmarksData, function($b) use ($ids) {
